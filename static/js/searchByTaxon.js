...
$("#searchByTaxon.nav-item").addClass( "active" );
          ...
          function success(relations) {
            ...
            thtable = $('#results_taxon_habitat').DataTable(
              ...
              $('#spinner_taxon_habitat2').hide();
                ...
                // add color for score
                $('#results_taxon_habitat').DataTable().rows().every(function() {
...
function filterColumnHabitat(i) {
  $('#results_taxon_habitat').DataTable().column(i).search(
    $('#habitat_th').val().replace(/;/g, "|"), true, false
  ).draw();
  checkURL();
}
$('input.column_filter').on( 'keyup click', function () {
  filterColumnHabitat($(this).parents('td').attr('data-column'));
} );
...
function filterColumnScore(column){
    ...
    $('#results_taxon_habitat').DataTable().draw();
    checkURL();
}
